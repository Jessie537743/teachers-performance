...
<div class="flex justify-between items-center gap-4 mb-5 flex-wrap animate-slide-up">
    <div>
        ...
        <p class="text-sm text-gray-500 mt-1">Predictive analytics for employee performance — system-wide insights, trends, and ML-based labels trained on self, dean, and peer evaluation history.</p>
    </div>
</div>
    ...
    <div class="p-5">
        <form method="GET" action="{{ route('analytics.index') }}" class="flex gap-3.5 items-end flex-wrap" id="analyticsFilterForm">
            ...
            <div class="m-0 min-w-[220px] flex-1 max-w-sm">
                <label class="block text-sm font-semibold text-slate-700 mb-1.5" for="faculty_name">Faculty name (table search)</label>
                <input type="text" name="faculty_name" id="faculty_name" autocomplete="off"
...
<div class="grid grid-cols-2 sm:grid-cols-4 gap-3.5 mb-5">
    <div class="stat-stagger animate-slide-up-delayed bg-white border border-gray-200 rounded-2xl shadow-sm p-5 transition hover:shadow-md">
        <div class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Faculty with evaluation data</div>
        <div class="text-2xl font-bold text-slate-900">{{ $analyticsFacultyWithDataCount }}</div>
        <div class="text-xs text-gray-400 mt-1">At least one of student / dean / self / peer for the selected period</div>
    </div>
    <div class="stat-stagger animate-slide-up-delayed bg-white border border-gray-200 rounded-2xl shadow-sm p-5 transition hover:shadow-md">
        <div class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Average GWA</div>
        <div class="text-2xl font-bold text-slate-900">{{ $analyticsAvgGwa !== null ? number_format($analyticsAvgGwa, 2) : '—' }}</div>
        <div class="text-xs text-gray-400 mt-1">Among faculty with data only</div>
    </div>
    <div class="stat-stagger animate-slide-up-delayed bg-white border border-gray-200 rounded-2xl shadow-sm p-5 transition hover:shadow-md">
        <div class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Top-tier performers</div>
        <div class="text-2xl font-bold text-slate-900">{{ $analyticsTopTierCount }}</div>
        <div class="text-xs text-gray-400 mt-1">Excellent or Outstanding (by personnel type)</div>
    </div>
    <div class="stat-stagger animate-slide-up-delayed bg-white border border-gray-200 rounded-2xl shadow-sm p-5 transition hover:shadow-md">
        <div class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Needs attention</div>
        <div class="text-2xl font-bold text-slate-900">{{ $analyticsNeedsAttentionCount }}</div>
        <div class="text-xs text-gray-400 mt-1">Low band per type (same tiers as HR intervention)</div>
    </div>
</div>
...
<div class="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden transition hover:shadow-md mb-5">
    ...
    <div class="p-5">
        <div class="relative h-[320px] md:h-[400px]">
            <canvas id="personnelTrendChart"
                    @class(['hidden' => empty($historicalTrend['labels'] ?? [])])></canvas>

            @if(empty($historicalTrend['labels'] ?? []))
                <div class="absolute inset-0 flex flex-col items-center justify-center text-center px-6">
                    ...
                    <p class="text-sm font-semibold text-slate-700">
                        @if($selectedPersonnelUser)
                            No historical evaluation data for {{ $selectedPersonnelUser->name }} yet
                        @else
                            Select a personnel above to see their trend
                        @endif
                    </p>
                    <p class="text-xs text-gray-500 mt-1.5 max-w-md leading-relaxed">
                        Trends appear here once evaluations are submitted across one or more periods. If this is a fresh install, run the sample data seeder or wait for an evaluation period to collect submissions.
                    </p>
                </div>
            @endif
        </div>
...
<div class="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden transition hover:shadow-md">
    <div class="px-5 py-3.5 border-b border-gray-200 flex justify-between items-center gap-3 flex-wrap">
        <div>
            ...
            <p class="text-xs text-gray-500 mt-1 max-w-3xl">Random Forest predictions (self/dean/peer features); requires school year, semester, ML API, and term feedback. Results are stored in <code class="text-xs bg-gray-100 px-1 rounded">faculty_predictions</code>.</p>
    ...
    <div class="overflow-x-auto">
        <table class="w-full border-collapse min-w-[700px]">
            ...
            <tbody>
                ...
                @empty
                <tr>
                    <td colspan="12" class="text-center text-gray-500 py-8 px-4">No data for the selected filters.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
