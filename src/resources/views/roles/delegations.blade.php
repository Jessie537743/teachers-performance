...
<div class="flex justify-between items-center gap-4 mb-5 flex-wrap animate-slide-up">
    <div>
        ...
        <p class="text-sm text-gray-500 mt-1">Temporarily grant a user's permissions to another user.</p>
    </div>
...
@if(session('success'))
    <div class="bg-green-100 border border-green-300 rounded-xl px-4 py-3 mb-4 text-green-800 text-sm">{{ session('success') }}</div>
@endif
@if(session('error'))
    <div class="bg-red-100 border border-red-300 rounded-xl px-4 py-3 mb-4 text-red-800 text-sm">{{ session('error') }}</div>
@endif
@if($errors->any())
    <div class="bg-red-100 border border-red-300 rounded-xl px-4 py-3 mb-4 text-red-800 text-sm">
                    ...
                    <select name="delegator_id" required class="w-full border border-gray-300 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                        ...
                        @foreach($users as $u)
                            <option value="{{ $u->id }}" {{ old('delegator_id') == $u->id ? 'selected' : '' }}>
                                {{ $u->name }} ({{ \App\Enums\Permission::roleLabel($u->role) }})
                            </option>
                        @endforeach
                    </select>
                </div>
                ...
                @forelse($delegations->items() as $d)
                    <tr>
                        <td class="px-6 py-3">
                            ...
                            <div class="text-xs text-gray-500">{{ \App\Enums\Permission::roleLabel($d->delegator->role ?? '') }}</div>
                        </td>
                        <td class="px-6 py-3">
                            <div class="font-semibold text-gray-800">{{ $d->delegatee->name ?? '—' }}</div>
                            <div class="text-xs text-gray-500">{{ \App\Enums\Permission::roleLabel($d->delegatee->role ?? '') }}</div>
                        </td>
                        <td class="px-6 py-3">
